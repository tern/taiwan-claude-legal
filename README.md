...
### 2. Claude Cowork 桌面版
如果您是使用 **Claude Cowork 桌面版**，您可以透過以下簡單步驟手動載入：

1. **下載本專案**：點擊本頁面右上角的 `Code` -> `Download ZIP` 並解壓縮。
2. **建立專案**：在 Claude.ai 首頁左側點擊 `Projects` -> `Create Project`。
3. **上傳知識庫**：將解壓後的 `laws/civil-code/articles.md` 檔案上傳至專案的 **Knowledge** 區塊。
4. **設定 Playbook**：將 `playbooks/taiwan-civil-code.md` 中的 `System Prompt` 內容複製到專案的 **Custom Instructions** 中。
