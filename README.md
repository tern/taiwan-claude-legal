...
### 1. Claude Code (終端機模式)
如果您正在使用 Anthropic 的 [Claude Code](https://github.com/anthropics/claude-code)，請直接在對話框中輸入：
...
### 2. Claude Cowork 桌面版
您可以透過以下方式將此插件加入您的 Cowork 工作區：

**方式一：透過 UI 安裝（推薦）**
1. 開啟 Claude Desktop，切換至 **Cowork** 標籤。
2. 點擊左側 **Customize** > **Browse plugins**。
3. 搜尋「taiwan-claude-legal」或選擇上傳本專案的壓縮檔。

**方式二：透過終端機（需已安裝 Claude Code）**
在終端機輸入以下指令：
```bash
claude plugin marketplace add tern/taiwan-claude-legal
claude plugin install taiwan-legal-audit
```
